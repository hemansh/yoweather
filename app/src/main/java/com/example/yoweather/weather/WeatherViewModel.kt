    ...
    fun getWeatherData() {
        coroutineScope.launch {
            ...
            try {
                ...
                _description.value = listResult.weather[0].description.toString()
                ...
                _sunrise.value = SimpleDateFormat(
                    "hh:mm a",
                    Locale.ENGLISH
                ).format(Date(listResult.sys?.sunrise?.times(1000)!!))
                _sunset.value = SimpleDateFormat(
                    "hh:mm a",
                    Locale.ENGLISH
                ).format(Date(listResult.sys?.sunset?.times(1000)!!))
